...
func PoCRewriteDockerHubImage(image string) (newImage string, err error) {
	cache := "harbor.renkulab.io/dockerhub_cache"
	...
	// Construct the image string using the cache prefix
	path := reference.Path(named)
	tagged, isTagged := named.(reference.Tagged)
	digested, isDigested := named.(reference.Digested)
	if isTagged && isDigested {
		tag := tagged.Tag()
		digest := digested.Digest()
		return fmt.Sprintf("%s/%s:%s@%s", cache, path, tag, digest), nil
	}
	if isTagged {
		tag := tagged.Tag()
		return fmt.Sprintf("%s/%s:%s", cache, path, tag), nil
	}
	if isDigested {
		digest := digested.Digest()
		return fmt.Sprintf("%s/%s@%s", cache, path, digest), nil
	}
	return fmt.Sprintf("%s/%s", cache, path), nil
}

type PoCRewriter struct{}

func (PoCRewriter) Rewrite(image string) (newImage string, err error) {
	return PoCRewriteDockerHubImage(image)
}
