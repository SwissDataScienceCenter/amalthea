...
class PrometheusMetric:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if type(metric_type) is str:
            ...
            raise ValueError(
                "Invalid type provided for metric_type, needed str or " f"PrometheusMetricType, got {type(metric_type)}"
            )
        if labelnames is None:
            labelnames = []
        sanitized_label_names = [self.sanitize_label_name(label) for label in labelnames]
        self._metric = self._metric_type.type(name, documentation, sanitized_label_names, *args, **kwargs)
    ...
    def __call__(
        ...
        if type(action) is str:
            ...
            raise ValueError(
                "Type of metric action needs to be either str or " f"PrometheusMetricAction, got {type(action)}"
            )
...
class PrometheusMetricHandler(MetricEventHandler):
    ...
    def _on_any_status_change(self, metric_event: MetricEvent):
        manifest_labels = self._collect_labels_from_manifest(metric_event.session)
        if metric_event.old_status == ServerStatusEnum.Starting and metric_event.status == ServerStatusEnum.Running:
            if metric_event.sessionCreationTimestamp:
                launch_duration = (metric_event.event_timestamp - metric_event.sessionCreationTimestamp).total_seconds()
